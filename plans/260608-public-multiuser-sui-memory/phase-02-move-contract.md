...
## Requirements
- **Functional:** `submit_prediction` (khoá lúc kickoff bằng Clock), `register_match`/`settle_match` (OracleCap), `record_scores` (OracleCap ghi shared Scoreboard), `submit_output_record` (user-owned Walrus/hash pointer), events đầy đủ.
- **Non-functional:** AdminCap (cold) tách OracleCap (hot, rotatable); grade() thuần để unit test; published mainnet, giữ UpgradeCap.

## Architecture
- **Objects:** owned `Prediction`(per user/match/kind); owned `OutputRecord`(per user output kind/blob/hash); shared `MatchRegistry`(Table<u64,Match>) — chỉ ghi khi register/settle, đọc-only khi submit; shared `Scoreboard`(Table<address,Score>) — OracleCap ghi sau chấm điểm.
...
- Sketch module đầy đủ có trong code; điểm v1: scoreline exact=10, correct result=3; non-scoreline dùng manual score khi oracle chấm.
...
2. Viết `prediction_game.move`: structs (AdminCap, OracleCap, MatchRegistry, Match, Scoreboard, Score, Prediction), events, `init`, entry fns.
...
6. `packages/contract/src/client.ts`: PTB wrappers (`buildSubmitPrediction`, `buildRecordScores`, `buildSettleMatch`, `buildRegisterMatch`) dùng `@mysten/sui` v2.17 `Transaction` + shared object ids + Clock `0x6`.

## Success Criteria
- [x] `sui move build` + `sui move test` pass (2026-06-09; current test count: 0).
- [x] Published **Sui Testnet**; ids ghi vào `.env.local`.
- [x] Chạy thử end-to-end: register_match → submit_prediction → record_scores → `Scored` emit (kiểm bằng GraphQL event indexer).
- [ ] Redeploy/upgrade testnet package after `OutputRecord` addition; current published package predates `submit_output_record`.
